module CF	
	class Base
		...
		def train
				Model.find(@modelID).update(message: '', state: :training, progress: 0, rmse: 0)
				train_do
		...
		def reset
				Model.find(@modelID).update(message: '', state: :resetting, progress: 0, rmse: 0)
				reset_do
		...
		def score
				Model.find(@modelID).update(message: '', state: :scoring, progress: 0, rmse: 0)
				score_do
